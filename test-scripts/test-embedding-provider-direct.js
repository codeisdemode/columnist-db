// Test Embedding Provider Integration (Direct Import)

console.log('🧪 Testing Embedding Provider Integration (Direct Import)...\n');

async function testEmbeddingProvider() {
  try {
    // Import directly from the manager file
    const { MemoryManager } = require('./packages/core/dist/memory/manager.js');
    const { BasicEmbeddingProvider } = require('./packages/core/dist/memory/embedding-providers/basic-embedding-provider.js');
